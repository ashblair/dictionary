...
#the executable is the name of the current working directory + the suffix:
...
ODIRS := $(ODIRS:./.%=) #like pattern substitution $(v:a=b) replaces all a w/ b
ODIRS := $(ODIRS:./%=%)
ODIRS := $(ODIRS:.=)
...
#initializing include directories:
IDIRS := $(shell find $(INC) -type d)
...
DEPS := $(filter $(addprefix $(BLD)/, $(addsuffix .d, $(SRCS))), $(DEPS))
...
$(BLD)/$(SRC)/%.o: $(SRC)/%
	@$(CXX) $(IDIRS) $< -MM -MT '$@' -MF $(subst .o,.d,$@)
	@#removing source file from depends file (sed script w/out slash & w/ double quotes for variable expansion):
	@sed -i "s| $<||g" $(subst .o,.d,$@)
	@$(CXX) $(CXX_FLAGS) $(IDIRS) $(CFLAGS) -c $< -o $@
...
logsetup:
	...
	@echo [INC=$(INC)] - [IDIRS=$(IDIRS)] - [DBG=$(DBG)] - [REL=$(REL)] >> $(LOGFILE)
